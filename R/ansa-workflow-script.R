...
BODATSADate<-"02 December 2024" # stipulate the date in the format shown here
...
### Some of the accepted species names for synonyms in the "Accepted" column are incorrectly formatted. Correct and flag where there were issues

FormatIssue<-BODATSA%>% filter(!Accepted %in% BODATSA$scientificName & !is.na(Accepted)) # Identify the species names in the "Accepted" column that are incorrectly formatted
...
for(i in 1:length(FormatIssue$Accepted)){ # loop through names with format issues
  ...
  if(length(b)==1){
  c<-z$scientificName[b] # take most similar match if there is only one closest match
  }
  ...
  names(issues)<-"scientificName"
   issues<-issues %>%
    ...
    d<-which(issues$words_correct == max(issues$words_correct)) # determine number of components in the potential matches that are equal to those in the species name
    if(length(d) == 1){
...
### Replace names in "Accepted" column with incorrect formatting with the correct names (those identified through fuzzy matching), and flag issues
...
BODATSA <- BODATSA %>% 
  rename("AcceptedIssues"= "Accepted") # rename the original "Accepted" column "AccpetedIssues"
...
BODATSA<-BODATSA %>%
  mutate(synAccIssue = ifelse(!is.na(fullScientificName), TRUE, FALSE)) # column is named "SunAccIssues"
...
NameType<-"canonical" # here stipulate 'canonical' or 'scientific'
...
# Taxonomic standardisation using GBIF backbone
...
## Step 10: Separate interim output into plants and other organisms

unique(GBIFMatch$kingdom) # list of kingdoms in the output. See below for an example

### Create a plants subset for further matching
...
names(PlantDat) # names of the columns. See below for an example of the plant subset

### Create a non-plants subset for further processing
...
### Look at what the results look like

head(GBIFMatch) # look at the first 5 rows of the dataset. See below for example
...
### Flag unrecognised taxa - those not recognised or only recognised at higher levels than the taxon (i.e., genus or above level for a species, or a species level for a sub-specific entity)

GBIFMatch = GBIFMatch %>% 
  mutate(unrecognised.flag=case_when((matchtype =="HIGHERRANK")~TRUE,
                                     (rank == "GENUS")~TRUE,
                                     (matchtype == "NONE") ~TRUE,
                                     TRUE~FALSE)) # give TRUE if not recognised

### Flag if the result should be checked for a possible error or issue - unrecognised, doubtful, and uncertain matches

GBIFMatch<-GBIFMatch %>% 
  mutate(possibleError.flag = ifelse(uncertain.flag =="TRUE" | doubtful.flag == "TRUE" | unrecognised.flag == "TRUE", TRUE, FALSE)) # give TRUE if there could be an error or issue

### Flag sub-specific entities

GBIFMatch = GBIFMatch %>% 
  group_by(rank) %>% 
  mutate(subspecific.flag = ifelse(rank =="SPECIES"|rank == "GENUS"|is.na(rank)|matchtype == "HIGHERRANK", FALSE, TRUE)) # give TRUE if sub-specific
...
GBIFMatch = GBIFMatch %>% 
  ...
  mutate(scientificNameduplicate.flag = ifelse(n() > 1 & rank !="GENUS" & rank !="KINGDOM" & rank !="PHYLUM" & rank !="CLASS" & rank != "ORDER" & rank != "FAMILY" & !is.na(rank),
                                               TRUE, FALSE)) # give TRUE if a duplicate

### Flag duplicates in verbatim names 

GBIFMatch = GBIFMatch %>% 
  group_by(verbatimScientificName) %>% 
  mutate(verbatimNameduplicate.flag = n() > 1) # give TRUE if a duplicate

### Flag duplicates in canonical names (exclude duplicates at higher taxonomic levels (e.g., genus))
...
### Add column with the source of the scientific name and the date accessed
...
names(GBIFMatch) # names of the columns. See below for an example

### Write interim output to file 

write.csv(GBIFMatch, "data//interim//NonPlantOrganisms_GBIF_standardisation.csv", row.names = FALSE)

# Taxonomic standardisation for plant taxa using BODATSA taxonomic backbone
...
#### based on scientific name (with authority)
...
BODATSAMatch = BODATSAMatch %>% 
  group_by(verbatimScientificName) %>% 
  mutate(multipleMatches = n() > 1) # give TRUE if there were multiple matches
...
#### Take the match that is an accepted scientifi name, if there is one
...
BODATSAMatch = BODATSAMatch %>% 
  group_by(verbatimScientificName) %>% 
  mutate(multipleMatchesUnresolved = n() > 1) # give TRUE if multiple matches are unresolved
...
#### Look at how many were assigned accepted scientific names that had been corrected through fuzzy matching (Step 4)  
...
### Flag taxon names that were not exact matches (i.e., unresolved multiple matches and accepted names corrected through fuzzy matching)

BODATSAMatch = BODATSAMatch %>% 
  mutate(uncertain.flag=case_when((multipleMatchesUnresolved =="TRUE")~TRUE,
                                  (synAccIssue =="TRUE")~TRUE,
                                  TRUE~FALSE)) # TRUE if unresolved, or fuzzy matching involved

### Flag doubtful taxa (uncertain, missapplied)

BODATSAMatch <- BODATSAMatch %>% mutate(doubtful.flag = str_detect(TaxStat, "unc|misap")) # TRUE if doubtful

#### Flag unrecognised taxa

BODATSAMatch = BODATSAMatch %>% 
  group_by(Genus) %>% 
  mutate(unrecognised.flag = ifelse(Genus =="NA", TRUE, FALSE)) # TRUE if unrecognised

### Flag if the result should be checked for a possible error or issue - unrecognised, doubtful, and uncertain matches

BODATSAMatch<-BODATSAMatch %>% 
  mutate(possibleError.flag = ifelse(uncertain.flag =="TRUE" | doubtful.flag == "TRUE" | unrecognised.flag == "TRUE", TRUE, FALSE)) # TRUE if possible error
...
BODATSAMatch<-BODATSAMatch %>% 
  mutate(subspecific.flag = ifelse(sppType > 2, TRUE, FALSE)) # TRUE if sub-specific entity
...
### Add column with the source of the scientific name and the date that BODATSA was downloaded
...
names(BODATSAMatch) # names of the columns. See below for an example
...
## Step 17: Match plant names that were not recognised by BODATSA with those in WCVP
...
#### Verbatim names are canonical (without authority)
if(NameType == "canonical"){
...
#### Verbatim names are scientific (with authority)
if(NameType == "scientific"){
  matches <- wcvp_match_names(PlantUnRec,
...
u<-which(wcvp_names$plant_name_id %in% matches$wcvp_accepted_id) # which rows in the WCVP dataset have the ids of the accepted species names
...
if(NameType == "scientific"){
  ...
  matchesAuth<-matchesAuth %>%
    filter(str_detect(verbatimScientificName, wcvp_authors)) # detect authorities in the verbatim name
  matchesAuth<-matchesAuth %>%
...
#### For remaining taxa with multiple matches, take the match with an accepted scientific name, if there is one

matchesMult<-matches%>% filter(multiple_matches == TRUE & wcvp_status == "Accepted") # subset of multiple matches, where there is an accepted species name
...
head(WCVPMatch) # Look at the first 5 rows of the data. See below for example
...
WCVPMatch<-WCVPMatch %>% 
  mutate(doubtful.flag = 
           ifelse(wcvp_status == "Accepted"|wcvp_status =="Synonym"|wcvp_status == "NA", "FALSE","TRUE")) # TRUE if doubtful

### Flag unrecognised taxa

WCVPMatch = WCVPMatch %>% 
  mutate(unrecognised.flag=ifelse(match_type =="NA", TRUE, FALSE)) # TRUE if not recognised

### Flag if the result should be checked for a possible error or issue - unrecognised, doubtful, and uncertain matches

WCVPMatch<-WCVPMatch %>% 
  mutate(possibleError.flag = ifelse(uncertain.flag =="TRUE" | doubtful.flag == "TRUE" | unrecognised.flag == "TRUE", TRUE, FALSE)) # TRUE if possible error or issue
...
### Add column with the source of the scientific name and date accessed
...
names(WCVPMatch) # names of the columns. See below for an example 

### Write interim output to file 

write.csv(WCVPMatch, "data//interim//Plants_WCVP_standardisation.csv", row.names = FALSE)

# Merge results into one final output file
...
### Remove unrecognised taxa from BODATSA match (these were matched using WCVP)
...
### Reorder the taxa in the output dataset, so that they are in the same order as they were in the input file

AllMatch<-AllMatch[match(NamesDat$verbatimScientificName, AllMatch$verbatimScientificName),]

# Step 21: Write output file
