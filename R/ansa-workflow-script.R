...
#### Replace names incorrectly formatted names in "Accepted" column with those identified through fuzzy matching, and flag
...
## Step 2b: Preparation of list of names
...
# Step 3: Match all names to GBIF backbone
...
### Flag unmatched taxa - those not matched or matched at higher levels than the taxon (i.e., genus or above level for a species, or a species level for a sub-specific entity)
...
GBIFMatch<-GBIFMatch %>% 
  mutate(possibleError.flag = ifelse(uncertain.flag =="TRUE" | doubtful.flag == "TRUE" | unmatched.flag == "TRUE", TRUE, FALSE)) # give TRUE if there could be an error or issue
...
# Step 10: Preparation of output for matched plants 
...
### Match if verbatim names are canonical (without authority)
if(NameType == "canonical"){
...
### Match if verbatim names are scientific (with authority)
if(NameType == "scientific"){
  matches <- wcvp_match_names(PlantUnMatched,
...
if(NameType == "scientific"){
  ...
  matchesAuth<-matchesAuth %>%
    filter(str_detect(verbatimScientificName, fixed(wcvp_authors))) # detect authorities in the verbatim name
  matchesAuth<-matchesAuth %>%
...
# Step 12: Assess and flag issues for remaining plants
...
## Step 13: Preparation of output for remaining plants
