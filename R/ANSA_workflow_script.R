...
for(i in 1:length(FormatIssue$Accepted)){ # loop through names with format issues
  ...
  fullScientificName<-rbind(fullScientificName, c) # bind the identified name through the fuzzy matching to result dataframe
}

FormatIssue<-cbind(FormatIssue,fullScientificName) # bind the matched names to those with format issues

### Replace names in "Accepted" column in incorrect format with the correct ones (those identified through fuzzy matching), and flag issues
...
for(j in 1:m){ # loop through the taxon names
...
unique(GBIFMatch$kingdom) # list of kingdoms in the dataset. See below for an example of output

### Create a plants subset for further matching

PlantDat<-GBIFMatch %>% filter(kingdom == "Plantae")  # create subset

PlantDat<-PlantDat %>% select(verbatimScientificName:family) # only keep the relevant columns
names(PlantDat) # names of the columns. See below for an example of the new output
...
GBIFMatch = GBIFMatch %>% 
  mutate(unrecognised.flag=case_when((matchtype =="HIGHERRANK")~TRUE,
                                     (rank == "GENUS")~TRUE,
                                     (matchtype == "NONE") ~TRUE,
                                     TRUE~FALSE)) # give TRUE is not recognised

### Flag if the result should be checked for a possible error or issue - unrecognised, doubtful, and uncertain matches

GBIFMatch<-GBIFMatch %>% 
  mutate(possibleError.flag = ifelse(uncertain.flag =="TRUE" | doubtful.flag == "TRUE" | unrecognised.flag == "TRUE", TRUE, FALSE)) # give TRUE if there could be an error
...
### Flag duplicates in returned scientific names (exclude duplicates if name is at higher levels (e.g., genus))
...
### Flag duplicates in canonical names (exclude duplicates at higher levels (e.g., genus))
...
### Add column with the source of the scientific name and the date
...
#### Handle taxa with multiple matches that could not be resolved
...
#### Concatenate the multiple results for the taxa and separate with pipe delimiter '|'
...
#### Look at how many were assigned accepted scientific names that had been corrected through fuzzy match (Step 4)  

BODATSAMatch %>%
  group_by(synAccIssue) %>%
  count() # TRUE are taxa with this issue

#### Look at how many had multiple matches
BODATSAMatch %>%
  group_by(multipleMatches) %>%
  count() # TRUE are taxa with multiple matches

#### Look at how many had multiple matches that were not resolved
BODATSAMatch %>%
  group_by(multipleMatchesUnresolved) %>%
  count()  # TRUE are taxa with unresolved multiple matches
...
BODATSAMatch = BODATSAMatch %>% 
  mutate(uncertain.flag=case_when((multipleMatchesUnresolved =="TRUE")~TRUE,
                                  ...
                                  TRUE~FALSE)) # TRUE if unresolved, or fuzzy matcching involved
...
### Add column with the source of the scientific name and date that BODATSA was downloaded
...
### Add column with the source of the scientific name and date
